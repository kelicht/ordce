...
class LinearOrderedActionExtractor():
    ...
    def extract(self, x, 
        ...
        # constraint: a_d = sum_{i} a_{d,i} pis_{d,i}
        if(intervention):
        ...
        if(ordering):
            ...
            use_sos1_constraint = False
            if(use_sos1_constraint):
                # auxiliary variables for sos-I constraints
                zeta = [[pulp.LpVariable('zta_{}_{}'.format(k,d), cat='Continuous') for d in range(self.D_)] for k in range(self.K_)]
                eta = [[pulp.LpVariable('ita_{}_{}'.format(k,d), cat='Binary') for d in range(self.D_)] for k in range(self.K_)]

                # constraint: zeta_{k,d} = epsilon_{k,d} - (sum_{i} a_{d,i} pi^(k)_{d,i} - delta_{k,d})
                # constraint: eta_{k,d} = 1 - sigma_{k,d}
                for k in range(self.K_):
                    for d in range(self.D_):
                        prob += epsilon[k][d] - pulp.lpDot(self.A_[d], pik[k][d]) + delta[k][d] - zeta[k][d] == 0
                        prob += sigma[k][d] + eta[k][d] == 1
                        prob.sos1['S_1_{}_{}'.format(k,d)] = sigma[k][d] + 2 * zeta[k][d]
                        prob.sos1['S_2_{}_{}'.format(k,d)] = eta[k][d] + 2 * epsilon[k][d]
        ...
        self.init_sols_['xi'] = [np.clip(xi[d].value(), self.x_[d]+self.lb_[d], self.x_[d]+self.ub_[d]) for d in range(self.D_)]
...
def _check_linear_oce(N=1, dataset='h', ordering=True):
    ...
    for n,x in enumerate(denied[:N]):
        ...
        oa = oce.extract(x, K=6, gamma=0.0, ordering=ordering, intervention=False, cost_type='DACE', ordering_cost_type='standard', log_name='./lp/linear')
        if(oa!=-1): print(oa)
        # oa = oce.extract(x, K=6, gamma=0.1, ordering=ordering, cost_type='TLPS', ordering_cost_type='standard', log_name='linear')
        # if(oa!=-1): print(oa)
        # oa = oce.extract(x, K=6, gamma=1.0, ordering=ordering, cost_type='TLPS', ordering_cost_type='standard', log_name='linear')
        # if(oa!=-1): print(oa)
        # oa = oce.extract(x, K=6, gamma=10.0, ordering=ordering, cost_type='TLPS', ordering_cost_type='standard', log_name='linear')
        # if(oa!=-1): print(oa)

if(__name__ == '__main__'):
    _check_linear_oce(N=5, dataset='d', ordering=True)
